...
# Making it an exportable function into main
def EXTRACT_DIRTY_DATA():
    import camelot.io as camelot
    import os
    import textract
    import json

    single_pdfs_path = "single_pdfs_dotted/"
    ...
    for pdf in pdfs:
        ...
        # Write this new name and replace an old one
        with open('single_user_data_dirty.json', 'w') as outfile:

            outfile.write(user_data)

        # Now read this into a var named user?
        with open("single_user_data_dirty.json", "r") as json_file:
        ...
        # Now add it to the final file.json and add year group
            ########## Update STUDENTS #############
        with open('json/dirty.json', 'r+') as file:
            data = json.load(file)
            data['STUDENTS'].update(user)
            data['STUDENTS'][name][0]['0'] = year
            file.seek(0)
